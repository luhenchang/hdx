...
//	}
	...
	public void initDetails() {
		...
		startProgress();
		httpCilents.get(httpCilents.printURL(Url.ACCUPASS_HOME_DETAIL, params), new WebServiceCallBack() {
...
//						if (isHuodong == 1) {
//							pushDetailsInfo = JSON.parseObject(response.result, PushDetailsInfo.class);
//							initJelly();
//						} else {
//							detailsInfo = JSON.parseObject(response.result, DetailsInfo.class);
//							initAccuvally();
//							
					...
					if (msg.isSuccess()) {
						...
						if (successInfo.getPrice() > 0) {
							...
							if (!successInfo.isNeedApply()) {
								...
								intent.setClass(AccuvallyDetailsActivity.this, PaySuccessActivity2.class);
								startActivity(intent);
							} else {
								application.showMsg(successInfo.getMsg());
							}
