...
public class LoginActivityNew extends BaseActivity implements OnClickListener, TextWatcher {
    ...
    public static  final int LeancloudLoginFinish=1;
    Handler handler=new Handler(){
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            switch (msg.what){
                case LeancloudLoginFinish:
                    stopProgressCheckNull();
                    finish();
                    break;
            }
        }
    ...
    private void initView() {
        ...
        if (application.sharedUtils.readString("userName") != null)//只在这里 userName 才有一点作用。
...
//                            application.showMsg("登录成功");
                            ...
                            handler.sendEmptyMessageDelayed(LeancloudLoginFinish, 3000);//
...
//                        application.sharedUtils.writeString(Keys.logintype, "logintype_three");
                            ...
                            EventBus.getDefault().post(new ChangeUserStateEventBus(ChangeUserStateEventBus.LOGIN));
                            postTag();//第三方登录
                            handler.sendEmptyMessageDelayed(LeancloudLoginFinish,3000);//THREE
                        } else {
                            application.showMsg(msgInfo.getMsg());
                        }
                        break;
